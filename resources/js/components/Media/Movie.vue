					...
					<div class="card-body">
						<div class="row">
							<div class="card_info_wrapper col-sm-12 col-lg-6">
								<h5 class="mb-4">
									<b>
										Show on prodution this movie -
										<a href="#" target="_blank">{{ movie.title }}</a>
									</b>
								</h5>

								<div class="card border-success" v-bind:class="[movie.status == 'active' ? 'border-success' : 'border-danger']">
									<div class="row" v-if="movie">
										<div class="image_wrapper">
											<span class="status" v-bind:class="[movie.status == 'active' ? 'badge-success' : 'badge-danger']">
												{{movie.status}}
											</span>
											<img
												v-if="movie.num_of_images > 0"
												v-bind:src="movie.coverUrl"
												alt="Movie cover"
											/>
											<img
												v-else
										...
										<p class="card-title col-12" v-for="(value, title) in movie">
											<b v-show="zeroShow"
												v-if="value == 0 && 
												value == '' &&
												value !== null">
											...
											<b v-if="title == 'licensor'">
												<span class="title">{{ title }}</span>: {{ value.name }} [{{ value.id }}] <span class="green">{{ value.status }}</span>
											</b>
